...
export function errorHandler(
  ...
  const knownErrors: Record<string, { message: string; status: number }> = {
    ...
    INVALID_CREDENTIALS: { message: "Credenciales inválidas", status: 401 },
    USER_NOT_FOUND: { message: "El usuario no existe", status: 401 },
    WRONG_PASSWORD: { message: "Contraseña incorrecta", status: 401 },
